...
public class AvlTreeMap<K extends Comparable<? super K>, V>
    ...
    private Node balance(Node n, K key) {
        int balance = this.height(n.right) - this.height(n.left);

        if (balance < -1) {
            if (this.height(n.left.right) - this.height(n.left.left) >= 1) {
                n.left = this.leftRotate(n.left);
            }
            return this.rightRotate(n);
        }
        if (balance > 1) {
            if (this.height(n.right.right) - this.height(n.right.left) <= -1) {
    ...
    private Node insert(Node n, K k, V v) {
        ...
        return this.heightAdj(this.balance(n, k));
    }
